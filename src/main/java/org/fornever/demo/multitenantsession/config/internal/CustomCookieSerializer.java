...
public class CustomCookieSerializer implements CookieSerializer {
    ...
    private String getCookieNameF() {
        ServletRequestAttributes attr = (ServletRequestAttributes) RequestContextHolder.currentRequestAttributes();
        HttpServletRequest request = attr.getRequest();
        List<String> parts = Arrays.stream(request.getRequestURI().split("/")).collect(Collectors.toList());
        return parts.get(parts.size() - 1) + this.cookieName;
    }
    ...
    public List<String> readCookieValues(HttpServletRequest request) {
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if (this.getCookieNameF().equals(cookie.getName())) {
    ...
    public void writeCookieValue(CookieValue cookieValue) {
        ...
        sb.append(this.getCookieNameF()).append('=');
